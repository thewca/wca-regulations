...
- A1a2++++) [ADDITION] If cumulative time limits for a single event or a combination of events are enforced, such events don't have regular time limits per solve.
- A1a2+++++) [CLARIFICATION] Events where time limit per solve is defined by Regulations (Fewest Moves Solving and Multiple Blindfolded Solving) can't have cumulative time limits.
