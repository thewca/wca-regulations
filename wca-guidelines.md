...
- 3d1b+) [CLARIFICATION] Patterns do not need to be present on every face, but only on faces where a distinct colour cannot be found. Patterns must not have any feature (e.g. textures, pattern irregularities) that significantly distinguishes them from similar pieces.
