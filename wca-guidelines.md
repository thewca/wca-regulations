...
- 2s++) [EXAMPLE] Competitors whose circumstances (e.g. religion) do not allow them to compete on an specific date or time may be allowed to compete in another period under the discretion of the WCA Delegate. The WCA Delegate must consider the well-being of the competition when attending such requests.
- 2s+++) [EXAMPLE] Competitors whose circumstances prevent them from following the correct procedures may be helped to do them. (e. g. a competitor that can not use one of their hands to start the timer may receive assistance from the judge to start and stop the timer, or a competitor that can not listen may receive their inspection warnings in a different way).
