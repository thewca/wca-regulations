...
- 1c3++) [Zusatz] Analoge Aufzeichnungen (z.B. Scoresheets, niedergeschriebene Fewest-Move-Lösungen) müssen mindestens einen Monat lang aufgehoben werden. Digitale Aufzeichnungen (z.B. Scrambles, Ergebnisse) müssen permanent aufgehoben werden.
...
- 4b2+) [Klarstellung] Im Allgemeinen sollten alle offiziellen Scrambles während dem Turnier geheim gehalten und nach dem Ende des Turniers gemeinsam veröffentlicht werden (siehe [Regel 1c3a](regulations:regulation:A2e1)). In manchen Fällen (z.B. bei Weltrekorden) kann das Organisationsteam gewisse Scrambles bereits nach dem Ende einer Runde herausgeben.
- 4b4+) [Zusatz] Die Orientierung des Puzzles darf sich auf dem Weg vom Scrambler zum Löse-Platz ändern, solange niemand versucht, die Zufälligkeit der Permutationen zu beeinflussen (siehe [Regel A2e1](regulations:regulation:A2e1))
...
- 4f++) [Erinnerung] Wenn der WCA-Delegierte zusätzliche Scrambles während des Turniers generiert, muss er sie speichern (siehe [Regel 1c3a](regulations:regulation:1c3a)).
...
- 9b3b+) [Klarstellung] Die "Mean of 3"-Statistik beeinflusst die Platzierung eines Teilnehmers in einer "Best of 3"-Runde (die auf dem besten erzielten Ergebnis basiert) nicht. Die WCA erkennt diese Statistiken unabhängig von dem Wettbewerbsformat an.
...
- 9f4+) [Erklärung] Das Ergebnis eines Versuchs ist nur dann DNF wenn der Teilnehmer den Versuch durch Kennzeichnen seiner Bereitschaft begonnen hat (siehe [Regel A3b2](regulations:regulation:A3b2)), und der Versuch disqualifiziert wurde.
...
- 11e1+) [Erinnerung] Ein Extraversuch muss vermischt werden, indem man einen unmodifizierten Scramble verwendet, der von einem offiziellen Scramble-Programm generiert wurde (siehe [Regel 4f](regulations:regulation:4f), [Richtlinie 4f+](guidelines:guideline:4f+)).


## <article-12><notation><notation> Article 12: Notation

- 12c2+) Beim Auswerten von eventuellen mangelnden Ausrichtungen am Ende eines Solves, werden x und y separat betrachtet. Beispiel: (5,1) zählt als eine mangelnde Ausrichtung, (5,5) zählt als zwei mangelnde Ausrichtungen.
...
- A6g+) [Zusatz] Während ein Judge überlegt, ob er auf Grund mangelnder Ausrichtung einer Seite eine "+2" Strafe geben muss, so sollte er das Puzzle dabei am besten überhaupt nicht berühren. Ausnahme: Für die Clock muss der Judge das Puzzle für gewöhnlich hochheben um beide Seiten begutachten zu können.
...
- H1d+) [Beispiel] Beispiel: Wenn ein Teilnehmer 10 Würfel versucht, mit einer Zeit von 59:57 abschließt und zwei Zeitstrafen hat, ist das Resultat: 59:57 + 2*2 = 60:01 (siehe [Regel A1a5](Regel A1a5)).
