...
- E2c1+++) [ADDITION] If a judge finds a solution without information to identify the attempt after the competitor has finished submitting it (e.g. while grading the attempt), they should find out the competitor who wrote the solution in order to assign them a DNF (rather than DNS). Note that the competitor must not be allowed to write information to identify the attempt at this point, so they must receive a DNF result.
