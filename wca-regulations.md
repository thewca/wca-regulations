...
- E3) The competitor may use the following objects during the attempt. Penalty for using unauthorized objects: disqualification of the attempt (DNF).
    - E3a) Solution Sheet (supplied by the judge), paper (supplied by the judge) with a picture of the scramble for that attempt, and pens/pencils or similar (supplied by the judge, or optionally self-supplied).
