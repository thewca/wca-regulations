...
- 4d) Scrambling orientation:
    - 4d1) NxNxN Cubes and Megaminx are scrambled starting with the white face (if not possible, then the lightest face) on the top and the green face (if not possible, then the darkest adjacent face) on the front.
...
- 12a) Notation for NxNxN Cubes:
    ...
    - 12a2) Outer Block Moves (outer slice plus adjacent inner slices). For each of the moves defined below, n is the total number of slices to move, which must be in the range 1 < n < N (where N is the number of layers in the puzzle). n may be also be omitted, for an implicit value of n == 2 slices. Outer Block Moves are:
...
- E2) Procedure for Fewest Moves Solving:
    ...
    - E2c) At 60 minutes, each competitor must submit to the judge a single piece of paper with the competitor's written name (optionally accompanied by WCA ID) and solution.
        ...
        - E2c2) The solution must be a single unambiguous move sequence written out as one or multiple horizontal lines of text. If the paper contains any moves that are not intended to be part of the solution, the competitor must box/circle a single move sequence as their intended solution. Penalty for additional moves, or for multiple boxes/circles around one or more moves each: disqualification of the attempt (DNF).
        - E2c3) The competitor may black out moves and other symbols by completely covering them in a solidly filled rectangle or other shape of ink. Blacked-out symbols are treated as if they are not on the page, and are not counted as part of the solution.
