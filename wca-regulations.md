...
- A1) Speed Solving attempts must abide by the following procedures.
    - A1a) The organization team applies exactly one kind of time limit (time limit per attempt or cumulative time limit) to each round.
        ...
        - A1a5) The time counting towards the time limit for an attempt is either the result after time penalties are applied (if the result was not DNF) or the time elapsed in the attempt (if the result was DNF).
