...
- A4) Starting the solve:
    ...
    - A4d) If a Stackmat timer is in use, the competitor should keep their hands on the timer until they see a green timer light. They start the solve by removing their hands from the timer and starting the timer.
...
- B2) Starting the attempt:
    ...
    - B2d) When using the Stackmat timer, the competitor starts the attempt by removing their hands from the timer and starting the timer. (This also starts the solve.)
