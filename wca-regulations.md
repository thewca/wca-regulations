...
- 3h) Modifications that enhance the basic concept of a puzzle are not permitted. Modified versions of puzzles are permitted only if the modification does not make any additional information available to the competitor (e.g. orientation or identity of pieces), compared to an unmodified version of the same puzzle.
    ...
    - 3h5) Customizations that leave the pins loose (i.e. pins that can toggle or recede using gravity instead of being directly pressed) are not considered reasonable wear, and these puzzles must not be permitted.
...
- F2) The judge places the scrambled puzzle onto the mat in a standing position.
    ...
    - F2b) If, during the transportation phase or while placing the puzzle cover on the table, one or more of the pins have their state affected, the competitor may proceed with the attempt and, after finishing the solve, alongside with the judge, explain the situation to the WCA Delegate (see [Guideline F1a1+++](guidelines:guideline:F1a1+++)). If the WCA Delegate believes that the competitor did not gain an unfair advantage, they may allow the result to stand under their discretion.
